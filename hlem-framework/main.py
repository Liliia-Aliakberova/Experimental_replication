...
def main(path, name, traffic_type='High', hlf_selected=hlem_with_log.DEFAULT_HLF, p=0.9, connection_thresh=0.5, res_info=True,
    ...
    frame = 'days'
    hl_log, df = hlem_with_log.transform_log_to_hl_log_width(name, log, frame, traffic_type, hlf_selected, p, connection_thresh,
...
if __name__ == '__main__':
    ...
    for number in range(1, num_parts + 1):
        my_path = os.path.join(current_dir, f"event_logs/part_{number}.xes")
        #print("my_path:", my_path)
        main(path=my_path, name=number)
